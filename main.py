...
def capture_virtual_screen() -> VirtualScreenCapture:
    ...
    with mss.mss() as sct:
        ...
        # Explicit bytes() keeps compatibility with MSS versions returning memoryview.
...
class SelectionOverlay:
    def __init__(
        ...
        geometry = (
            ...
            f"{capture.left:+d}{capture.top:+d}"
        )
...
class OCRCaptureApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        print(
            "모드:",
            "OCR 후 자동 붙여넣기" if self.config.get("auto_paste", True)
            else "OCR 후 클립보드 복사만",
        )
    ...
    def _configure_tesseract(self) -> None:
        ...
        if missing:
            messagebox.showerror(
                ...
                "OCR 언어 데이터가 없습니다: "
                + ", ".join(missing)
                + "\n\nTesseract tessdata 폴더에 해당 traineddata를 설치해 주세요.",
            )
            raise SystemExit(1)
